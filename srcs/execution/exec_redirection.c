...
static void check_permission(t_command *command, int write_flag)
{
	...
	if (write_flag == 0) // lecture
	{
		if (access(redir->filename, F_OK) != 0 || access(redir->filename, R_OK) != 0)
		{
			...
			ft_exit_free(redir->filename, EXIT_FAILURE, command, NULL);
		}
	}
	else // écriture
	{
...
void	infile_redirection(t_command *command)
{
	...
	while (cur)
	{
		if (cur->type == REDIR_IN && cur->filename)
		{
			...
			if (fd == -1)
				ft_exit_free(cur->filename, EXIT_FAILURE, command, NULL);
		}
		cur = cur->next;
	}
	if (fd != -1)
	{
		if (dup2(fd, STDIN_FILENO) == -1)
			ft_exit_free("dup2 input", EXIT_FAILURE, command, NULL);
		close(fd);
	}
}
...
void	heredoc_redirection(t_command *command)
{
	...
	while (cur)
	{
		if (cur->type == REDIR_HEREDOC)
		{
			...
			if (fd == -1)
				ft_exit_free("heredoc", EXIT_FAILURE, command, NULL);
		}
		cur = cur->next;
	}
	if (fd != -1)
	{
		if (dup2(fd, STDIN_FILENO) == -1)
			ft_exit_free("dup2 heredoc", EXIT_FAILURE, command, NULL);
		close(fd);
	}
}
...
void	outfile_truncate_redirection(t_command *command)
{
	...
	if (fd != -1)
	{
		if (dup2(fd, STDOUT_FILENO) == -1)
			ft_exit_free("dup2 output", EXIT_FAILURE, command, NULL);
		close(fd);
	}
}
...
void	outfile_append_redirection(t_command *command)
{
	...
	{
		if (dup2(fd, STDOUT_FILENO) == -1)
			ft_exit_free("dup2 append", EXIT_FAILURE, command, NULL);
		close(fd);
	}
}
